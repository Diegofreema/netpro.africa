...
export function HeroStatsPanel({ className = '' }: HeroStatsPanelProps) {
  return (
    <div
      className={`np-hero-stats-panel grid grid-cols-2 gap-y-8 rounded-3xl border border-white/15 bg-[oklch(0.22_0_0/0.82)] px-6 py-7 text-center shadow-[0_24px_80px_oklch(0_0_0/0.35)] backdrop-blur-md sm:grid-cols-4 sm:px-8 xl:rounded-[1.75rem] xl:px-12 xl:py-9 ${className}`.trim()}
    >
      {heroStats.map((stat) => (
        <div
          key={stat.label}
