...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useEffect(() => {
    const markFontsLoaded = () =>
      window.requestAnimationFrame(() => setFontsLoaded(true));
  ...
  useGSAP(
    () => {
      ...
      const restoreFinalText = () => {
        wrappersRef.current = [];
        try {
          splitRef.current?.revert();
        } catch {
          // GSAP can throw if SplitText was already reverted during rapid rebuilds.
        }
        splitRef.current = null;
        el.textContent = text;
        if (colorTo) el.style.color = colorTo;
      ...
      const build = () => {
        ...
        chars.forEach((ch) => {
          ...
          firstOrig.setAttribute('aria-hidden', 'true');
          firstOrig.className =
            'text-left ' +
            (shuffleDirection === 'up' || shuffleDirection === 'down'
              ? 'block'
          ...
          ch.setAttribute('data-shuffle-final', '1');
          ch.className =
            'text-left ' +
            (shuffleDirection === 'up' || shuffleDirection === 'down'
              ? 'block'
          ...
          for (let k = 0; k < rolls; k++) {
            ...
            c.removeAttribute('data-shuffle-final');
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            ...
            if (isVertical) {
              gsap.set(strips, {
                y: (_index: number, t: HTMLElement) =>
                  parseFloat(t.getAttribute('data-start-y') || '0'),
              });
            } else {
              gsap.set(strips, {
                x: (_index: number, t: HTMLElement) =>
        ...
        const addTween = (targets: HTMLElement[], at: number) => {
          ...
          if (isVertical) {
            vars.y = (_index: number, t: HTMLElement) =>
              parseFloat(t.getAttribute('data-final-y') || '0');
          } else {
            vars.x = (_index: number, t: HTMLElement) =>
