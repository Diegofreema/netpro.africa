...
import { careersCta } from '@/features/careers/data/careers-page-content'
import { assetUrl } from '@/shared/config/site'

export function CareersCta() {
  return (
    <section className="np-container pb-24 pt-6 sm:pt-12">
      <div className="np-card-texture relative overflow-hidden rounded-[var(--np-radius-xl)] border border-white/10 px-6 pt-18 text-center sm:px-10 lg:pt-24" data-reveal="media">
        <div className="relative z-10 mx-auto flex max-w-5xl flex-col items-center gap-6">
          ...
          <p className="max-w-4xl text-sm leading-7 text-muted-foreground sm:text-base">{careersCta.description}</p>
          <p className="mt-5 text-base font-medium text-foreground">{careersCta.prompt}</p>
          <div className="flex flex-col gap-4 sm:flex-row">
            <LandingButton to="/contact-us">{careersCta.primary}</LandingButton>
            <LandingButton to="/projects" variant="quiet">
              {careersCta.secondary}
            </LandingButton>
          </div>
        </div>
