...
export function ThemeToggle() {
  ...
  return (
    <button
      className="np-focus fixed bottom-5 right-5 z-50 inline-flex min-h-14 items-center gap-3 rounded-full border border-(--np-border-strong) bg-(--np-toggle-bg) p-2 pl-5 text-base font-semibold text-muted-foreground shadow-(--np-shadow-soft) backdrop-blur transition-[background-color,border-color,color,transform] duration-(--np-duration-fast) hover:text-foreground active:scale-[0.98] sm:bottom-8 sm:right-8"
      type="button"
    ...
    >
      ...
      <span className="inline-flex size-11 items-center justify-center rounded-full bg-(--np-toggle-knob) text-foreground">
