  ...
  return (
    <section className="np-container py-(--np-section-y)">
      ...
      <div className="mt-16 grid gap-10 lg:grid-cols-[18rem_1fr]">
        <div
          className="flex flex-wrap gap-4 self-start lg:sticky lg:top-32 lg:flex-col"
          data-reveal="media"
        >
          {expertiseTabs.map((tab) => {
            ...
            return (
              <Button
                ...
                className={cn(
                  'min-h-14 justify-start rounded-(--np-radius-md) px-5 text-base',
                  !isActive &&
                    'border-white/10 bg-white/2.5 text-foreground hover:bg-white/6',
                )}
        ...
        <div data-reveal="media">
          ...
          <p className="mt-5 max-w-5xl text-base leading-7 text-muted-foreground sm:text-lg">
            {activeTab.summary}
          </p>

          <div className="mt-12 rounded-(--np-radius-xl) border border-white/10 bg-(--np-surface) p-6 sm:p-8 lg:p-10">
            <div className="grid gap-8 md:grid-cols-2">
              {activeTab.sections.map((section) => (
                <article
                  key={section.title}
                  className="border-white/10 md:border-r md:odd:border-r md:even:border-r-0 md:nth-[n+3]:border-t md:nth-[n+3]:pt-8"
                >
